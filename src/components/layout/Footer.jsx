...
export default function Footer() {
  ...
  // Logic to handle menu data fallback
  const navLinks = menu || [];
      ...
      <Container>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-16 lg:gap-12 relative z-10">
          ...
          <aside className="space-y-8">
            <div className="space-y-6">
              ...
              <p className="text-sm font-light leading-relaxed text-slate-500 max-w-xs">
                Empowering the youth of Varanasi and Nandgram through digital literacy, nutrition, and sustainable growth. Join our journey to make a difference.
              </p>
            </div>

            {/* Newsletter: High-End Look */}
            <div className="space-y-4">
               <h6 className="text-[10px] font-bold text-white/40 uppercase tracking-widest">Newsletter</h6>
          ...
          <nav className="space-y-8 lg:pl-10">
            <h6 className="font-heading font-bold text-white/30 uppercase tracking-[0.3em] text-[10px]">Quick Links</h6>
            <ul className="flex flex-col gap-4 text-[13px] font-bold tracking-tight">
              {navLinks.map((item) => (
                <li key={item.name}>
                  <Link href={item.href} className="hover:text-primary transition-all duration-300 flex items-center group uppercase tracking-widest text-[11px]">
...
<aside className="space-y-8">
  <h6 className="font-heading font-bold text-white/30 uppercase tracking-[0.3em] text-[10px]">Reach Us</h6>
  <div className="space-y-6">
    ...
    <div className="flex items-start gap-4 group">
      ...
      <div className="border-l border-white/5 pl-4">
        ...
        <span className="text-[9px] text-slate-600 uppercase tracking-widest font-bold">Mon - Sat (10am - 6pm)</span>
      </div>
    </div>
    ...
    <div className="flex items-start gap-4 group">
      ...
      <div className="border-l border-white/5 pl-4">
        ...
        <span className="text-[9px] text-slate-600 uppercase tracking-widest font-bold">Official Inquiry</span>
      </div>
    </div>
    ...
    <div className="flex items-start gap-4 group">
      ...
      <div className="border-l border-white/5 pl-4">
        <p className="text-[12px] leading-relaxed text-slate-500 font-light italic">
          ...
          {/* Section 4: Connect & Authority */}
          <nav className="space-y-10 lg:pl-6">
            <div className="space-y-6">
              <h6 className="font-heading font-bold text-white/30 uppercase tracking-[0.3em] text-[10px]">Social Ecosystem</h6>
              <div className="flex flex-wrap gap-3">
                {social.map((item, i) => (
                  <Link 
                    key={i} 
                    href={item.link} 
                    className={`h-11 w-11 flex items-center justify-center rounded-full bg-white/5 text-white transition-all duration-500 ${item.color} hover:shadow-[0_10px_20px_-5px_rgba(0,0,0,0.5)] hover:-translate-y-1.5`}
                  >
            ...
            <div className="p-6 rounded-[2rem] bg-gradient-to-br from-white/[0.03] to-transparent border border-white/5 backdrop-blur-md relative group overflow-hidden">
               ...
               <span className="text-[9px] font-black text-primary block uppercase tracking-[0.3em] mb-2">NGO Certification</span>
               <span className="text-xs font-mono font-bold text-slate-200 tracking-wider">REG: 12345/NGO/GZB</span>
            </div>
          </nav>
        </div>
        ...
        <div className="mt-24 pt-10 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-8">
          <div className="flex flex-col md:flex-row items-center gap-4 text-[10px] font-bold text-slate-600 uppercase tracking-[0.2em] text-center md:text-left">
            <p>© {new Date().getFullYear()} Digital Duniyaa Foundation</p>
          ...
          <div className="flex items-center gap-3 py-2 px-6 bg-white/[0.02] border border-white/5 rounded-full">
            <span className="text-[9px] font-bold text-slate-600 uppercase tracking-widest">Digital Architect</span>
            <a className="text-[10px] font-black text-white hover:text-primary transition-all tracking-[0.2em] cursor-pointer">
               BETTER OKAY
            </a>
          </div>
        </div>
