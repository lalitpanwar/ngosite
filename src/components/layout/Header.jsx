...
export default function Header() {
  ...
  const navLinks = menu || [];
  const { contact } = SITE_CONFIG; // Destructure contact data
          ...
          <div className="flex items-center justify-between h-16 md:h-20">
            ...
            <div className="flex items-center gap-3 md:gap-4">
              ...
              <Link href="/" className="flex items-center gap-2 group">
                <div className="relative h-12 w-12 md:h-16 md:w-16 shrink-0 filter drop-shadow-[0_4px_6px_rgba(0,93,80,0.1)] transition-all duration-300">
                  <Image 
                    src="/ddfoundation.jpg" 
                ...
                <div className="flex flex-col justify-center">
                  <h2 className="text-xs md:text-xl font-heading font-black tracking-tighter text-slate-900 uppercase leading-none">
                    DIGITAL <span className="text-primary italic">DUNIYAA</span>
                  </h2>
                  <div className="flex items-center justify-center gap-1 mt-1">
                    <span className="h-[1px] w-2 bg-slate-300"></span>
            ...
            <nav className="hidden lg:flex items-center gap-8">
              {navLinks.map((link) => (
                <Link 
                  key={t(link.name)} 
                  href={link.href}
                  className="text-xs uppercase font-heading font-bold text-slate-700 hover:text-primary transition-colors"
                >
                  {t(link.name)} 
                </Link>
              ))}
        ...
        <div className={`fixed top-0 left-0 h-full w-[280px] bg-white z-[70] lg:hidden transform transition-transform duration-500 ease-in-out shadow-2xl ${isOpen ? "translate-x-0" : "-translate-x-full"}`}>
          <div className="flex flex-col h-full p-6">
            <div className="flex items-center justify-between mb-10 border-b border-slate-50 pb-4">
              <span className="font-heading font-black text-lg text-primary tracking-tighter uppercase">Menu</span>
              <button onClick={() => setIsOpen(false)} className="p-2 text-slate-400 hover:text-primary transition-colors">
                <FaXmark size={24} />
              </button>
            </div>
            ...
            {/* Drawer Footer (Using Constants) */}
            <div className="mt-auto space-y-4 pt-6 border-t border-slate-100">
               <div className="text-[10px] uppercase tracking-widest text-slate-400 font-bold mb-2">Get in touch</div>
               <a href={contact?.phone[0].link} className="flex items-center gap-3 text-sm text-slate-600 font-medium">
                  <FaPhone className="text-primary text-xs" /> {contact.phone[0].no}
               </a>
               <a href={contact?.email[0].link} className="flex items-center gap-3 text-sm text-slate-600 font-medium">
                  <FaEnvelope className="text-primary text-xs" /> {contact.email[0].emailid}
               </a>
               ...
               <div className="mt-4 p-4 bg-primary/5 border border-primary/10 rounded-2xl">
                  ...
                  <p className="text-xs text-slate-600 font-medium">1,000+ Lives touched</p>
               </div>
            </div>
          </div>
        </div>
